...
void Assets::loadFromFile(const std::string & path)
{
    ...
    while (file.good())
    {
        ...
        if (str == "Texture")
        {
            std::string name, path, smooth;
            file >> name >> path >> smooth;
            addTexture(name, path, std::stoi(smooth));
        }
        else if (str == "Animation")
        {
            ...
            size_t frames, speed;
            bool override;
            file >> name >> texture >> frames >> speed >> override;
            addAnimation(name, texture, frames, speed, override);
        }
        else if (str == "Sound")
        {
...
void Assets::addAnimation(const std::string & animationName, const std::string & textureName, size_t frameCount, size_t speed, bool override)
{
    m_animationMap[animationName] = Animation(animationName, getTexture(textureName), frameCount, speed, override);
}
